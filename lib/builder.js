...
function review(state) {
  ...
  return { status: 200, content: `<section class="builder-review"><h2>Ready to execute</h2><p>This page has not sent anything. Following <strong>Execute request</strong> will make a real POST. Retries and link scanners can repeat the action; use an upstream idempotency key when available.</p>
    ...
    <p class="hint">Execute request uses /api/post on this page’s current origin (the same scheme, hostname, and port). Resolve its relative URL against this page’s URL; keep the current domain. The final link uses the existing converter and logging path. If the operator enables an API key, link-only clients without an Authorization header cannot execute it.</p></section>` };
}
