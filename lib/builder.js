import { MAX_URL_BYTES, parseRequest, upstreamHeaders, ProxyError } from './proxy.js';

const names = ['destination', 'body', 'headers', 'response', 'timeout', 'method'];
const groups = {
...
const initial = () => ({ v: 2, fields: ['', '', '{}', 'json', '15000', 'POST'], edit: null });
...
const editable = (index) => Number.isInteger(index) && [0, 1, 2, 4, 5].includes(index);
...
function decode(token) {
  ...
  if (!state || Array.isArray(state) || typeof state !== 'object'
    || Object.keys(state).sort().join(',') !== 'edit,fields,v' || ![1, 2].includes(state.v)
    || !Array.isArray(state.fields) || state.fields.length !== (state.v === 1 ? 5 : 6) || !state.fields.every(isText)
    || !['raw', 'json'].includes(state.fields[3])) return badState();
  if (state.v === 1) {
    if (state.edit?.[0] === 5) return badState();
    state = { ...state, v: 2, fields: [...state.fields, 'POST'] };
  }
...
function editor(state, group) {
  ...
  return `<section class="builder-editor"><h2>Editing ${escape(names[state.edit[0]])}</h2>
    ...
    ${state.edit[0] === 5 && group !== 'unicode' ? `<nav class="builder-links" aria-label="HTTP methods">${['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS', 'TRACE', 'CONNECT'].map((method) => link(`Set ${method}`, draft(state, method), 'edit')).join(' ')}</nav>` : ''}
...
function review(state) {
  const [url, data, headers, response, timeout, method] = state.fields;
  const requestUrl = `/api/post?${new URLSearchParams({ url, data, headers, response, timeout, method })}`;
  let config;
  ...
  return { status: 200, content: `<section class="builder-review"><h2>Ready to execute</h2><p>This page has not sent anything. Following <strong>Execute request</strong> will send a real ${escape(config.method)} request. Retries and link scanners can repeat the action; use an upstream idempotency key when available.</p>
    <h3>${escape(config.method)} ${escape(config.target.href)}</h3><h3>Effective upstream headers</h3><pre>${escape(JSON.stringify(upstreamHeaders(config), null, 2))}</pre>
    ${config.method === 'CONNECT' ? '<p>CONNECT returns only handshake status and headers, then closes the connection. It does not relay a tunnel.</p>' : ''}
    <p>Body, response mode, and timeout are shown above. Destination DNS and network checks run at execution. Redirects are not followed. A timeout does not undo the request.</p>
...
export function renderBuilder(requestUrl) {
  try {
    ...
    else content = `<section><h2>Build your request</h2><nav class="builder-links">${editingLinks(state)} ${link('Review request', state, 'review')} <a href="/builder">Start over</a></nav><p>Edit method offers standard HTTP methods and accepts custom method tokens. POST is the default. Headers are edited as a JSON object with string values. The default is an empty object; the converter adds Content-Type: application/json. For a JSON body, clear the field if needed and use JSON fragments plus letter links. Review will validate the complete request before offering execution.</p></section>`;
