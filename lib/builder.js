...
function href(state, view = 'summary', group = 'url') {
  ...
  if (view === 'edit') params.set('group', group);
...
function editingLinks(state) {
  ...
  return names.map((name, index) => index === 3 ? link('Change response mode', saved, 'response')
    : link(`Edit ${name}`, { ...saved, edit: [index, saved.fields[index], ''] }, 'edit', index === 4 ? 'numbers' : index === 0 ? 'url' : 'json')).join(' ');
}
...
function editor(state, group) {
  ...
  } else tokens = `<nav class="builder-links builder-tokens" aria-label="Append token">${groups[group][1].map(append).join(' ')}</nav>`;
  return `<section class="builder-editor"><h2>Editing ${escape(names[state.edit[0]])}</h2>
    ...
    ${state.edit[0] === 4 ? `<nav class="builder-links" aria-label="Timeout presets">${['1000', '5000', '15000', '20000'].map((value) => link(`Set ${value} ms`, draft(state, value), 'edit', group)).join(' ')}</nav>` : `<h3>Suggested fragments</h3><nav class="builder-links">${suggestions(state).map(append).join(' ')}</nav>`}
    <h3>Choose tokens</h3><nav class="builder-links" aria-label="Token groups">${Object.entries(groups).map(([key, [label]]) => link(label, state, 'edit', key)).join(' ')}</nav>
    <h3>${escape(groups[group][0])}</h3>${tokens}
