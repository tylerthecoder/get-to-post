...
// Only trust forwarding headers inside Vercel's managed runtime. Elsewhere,
// the direct socket peer is the only address we can establish ourselves.
function clientAddress(req) {
  if (process.env.VERCEL === '1') {
    const value = req.headers?.['x-forwarded-for'];
    if (typeof value === 'string' && value.length <= 1024) {
      const ip = value.split(',', 1)[0].trim();
      if (!ip.includes('%') && isIP(ip)) return { clientIp: ip, clientIpSource: 'x-forwarded-for' };
    }
    // A Vercel socket peer is infrastructure, not necessarily the caller.
    return { clientIp: null, clientIpSource: null };
  }
  const ip = req.socket?.remoteAddress;
  if (typeof ip === 'string' && !ip.includes('%') && isIP(ip)) {
    return { clientIp: ip, clientIpSource: 'socket' };
  }
  return { clientIp: null, clientIpSource: null };
}
  ...
  return {
    async start(record) {
      await execute(`INSERT INTO request_logging.requests
        (id, received_at, method, path, query, headers, url_bytes, truncated, environment, deployment, client_ip, client_ip_source)
        VALUES ($1, $2, $3, $4, $5::jsonb, $6::jsonb, $7, $8, $9, $10, $11::inet, $12)`, [
        ...
        record.truncated, record.environment, record.deployment, record.clientIp, record.clientIpSource,
      ]);
    },
