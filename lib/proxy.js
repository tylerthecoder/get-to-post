...
export function parseRequest(requestUrl) {
  ...
  const allowed = new Set(['url', 'data', 'headers', 'response', 'timeout', 'method']);
  ...
  const suppliedMethod = params.get('method') ?? 'POST';
  if (!suppliedMethod || /[^!#$%&'*+.^_`|~0-9A-Za-z-]/.test(suppliedMethod)) fail(400, 'invalid_method', 'method must be a nonempty HTTP method token.');
  const method = suppliedMethod.toUpperCase();
  const data = params.get('data') ?? '';
  if (['TRACE', 'CONNECT'].includes(method) && data) fail(400, 'invalid_data', `${method} does not support a request body.`);
  let target;
  ...
  if (method === 'CONNECT' && (target.pathname !== '/' || target.search)) fail(400, 'invalid_url', 'CONNECT requires an HTTPS origin without a path or query.');
  ...
  return { target, hostname, headers, data, method, mode, timeout };
}
...
export function upstreamHeaders(config) {
  return config.method === 'CONNECT' || config.method === 'TRACE'
    ? { ...config.headers }
    : { ...config.headers, 'content-length': Buffer.byteLength(config.data) };
}

export async function requestUpstream(config, { resolver = lookup, transport = https.request } = {}) {
  const signal = AbortSignal.timeout(config.timeout);
  let onAbort;
  const aborted = new Promise((_, reject) => {
    onAbort = () => reject(new ProxyError(504, 'upstream_timeout', 'Upstream request timed out. It may already have processed the request; do not retry blindly.'));
  ...
  try {
    return await Promise.race([aborted, (async () => {
      ...
      return await new Promise((resolve, reject) => {
        const request = transport(config.target, {
          method: config.method, agent: false, signal,
          ...(config.method === 'CONNECT' ? { path: `${config.target.hostname}:443` } : {}),
        ...
        // CONNECT returns a handshake, never a usable tunnel or tunnel bytes.
        ...
        request.on('upgrade', (_response, socket) => {
          socket.destroy();
          reject(new ProxyError(502, 'unsupported_upgrade', 'Upstream protocol upgrades are not supported.'));
        });
    ...
    if (signal.aborted) fail(504, 'upstream_timeout', 'Upstream request timed out. It may already have processed the request; do not retry blindly.');
